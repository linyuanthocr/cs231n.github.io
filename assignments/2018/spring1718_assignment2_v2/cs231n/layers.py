...
def affine_forward(x, w, b):
    ...
    - cache: (x, w, b)
    """
#     print("affine_forward:x shape: ",x.shape)
#     print("affine_forward:w shape: ",w.shape)
#     print("affine_forward:b shape: ",b.shape)
    out = None
    N = x.shape[0]
    ###########################################################################
...
#     print("dout shape: ",dout.shape)
    x, w, b = cache
#     print("affine_backward:x shape: ",x.shape)
#     print("affine_backward:w shape: ",w.shape)
#     print("affine_backward:b shape: ",b.shape)
...
def relu_forward(x):
    ...
    - cache: x
    """
#     print("relu_forward:x shape: ",x.shape)
    out = None
    ###########################################################################
...
def relu_backward(dout, cache):
    ...
    """
#     print("relu_backward:x shape: ",dout.shape)
    dx, x = None, cache
    ###########################################################################
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        out = scale*x+(beta-running_mean*scale)
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
    else:
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    gamma, x, smean, svar, eps, x_hat = cache
    N = x.shape[0]
    dxhat = dout*gamma
    dvarb = np.sum(dxhat*(x-smean),axis=0)*(-0.5)*(svar+eps)**(-1.5)
    dmeanb = np.sum(dxhat*(-1)*(svar+eps)**(-0.5),axis=0)-2*dvarb*np.mean(x-smean,axis=0)
    dx = dxhat*(svar+eps)**(-0.5)+2*dvarb*(x-smean)/N+dmeanb/N
...
def batchnorm_backward_alt(dout, cache):
    ...
    ###########################################################################
    gamma, x, running_mean, running_var, eps, x_hat = cache
    N = x.shape[0]
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for f in range(F):
            for lh in range(Hout):
                for lw in range(Wout):
                    out[n,f,lh,lw] = np.sum(x_pad[n,:,lh*stride:lh*stride+HH,lw*stride:lw*stride+WW]*w[f,:])+b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(N):
        for f in range(F):
            ...
            for lh in range(Hout):
                for lw in range(Wout):
                    dw[f] += x_pad[i,:,lh*stride:lh*stride+HH,lw*stride:lw*stride+WW]*dout[i,f,lh,lw]
                    dx_pad[i,:,lh*stride:lh*stride+HH,lw*stride:lw*stride+WW] += w[f]*dout[i,f,lh,lw]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
        for c in range(C):
            for h in range(Hout):
                for w in range(Wout):
                    out[i,c,h,w] = np.max(x[i,c,h*stride:h*stride+ph,w*stride:w*stride+pw])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for c in range(C):
            for h in range(Hout):
                for w in range(Wout):
                    ind = np.unravel_index(np.argmax(x[i, c, h*stride:h*stride+ph,
                                                       w*stride:w*stride+pw], axis=None), (ph, pw))
                    dx[i,c,h*stride:h*stride+ph,w*stride:w*stride+pw][ind] = dout[i,c,h,w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
